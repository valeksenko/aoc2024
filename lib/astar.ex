defmodule AoC2024.Astar do
  alias Astar.HeapMap
  require HeapMap.Pattern

  @moduledoc """
    A* graph pathfinding from https://github.com/wkhere/eastar/blob/master/lib/astar.ex
    Modified version to provide access to the collected vertices
  """
  ...
  * `goal`  - vertex we want to reach, or a function of arity 1 to check
  ...
  defp loop({nbs, dist, h} = env, goal, openmap, closedset, parents) do
    ...
    if has_reached_goal?(x, goal) do
      ...
      {openmap, parents} =
        Enum.reduce(nbs.(x, cons_path(parents, x)), {openmap, parents}, fn y,
                                                                           {openmap, parents} =
                                                                             continue ->
